...
export function MaintenanceAlertsWidget() {
  const { schedules, isLoading, machines } = useTPM();

  const getMachineName = (machineId: string) => {
    return machines.find(m => m.id === machineId)?.code || 'MÁQ';
  };

  // Filter for schedules that are due or overdue
  const pendingMaintenance = schedules.filter(s => 
    s.status === 'due' || s.status === 'overdue'
  ).sort((a, b) => {
    if (a.status === 'overdue' && b.status !== 'overdue') return -1;
    if (a.status !== 'overdue' && b.status === 'overdue') return 1;
    return new Date(a.next_maintenance).getTime() - new Date(b.next_maintenance).getTime();
  });
            ...
            {pendingMaintenance.map((schedule) => (
              ...
              >
                <div className={cn(
                  ...
                  schedule.status === 'overdue' ? "bg-destructive/20 text-destructive" : "bg-primary/20 text-primary"
                )}>
                  {schedule.status === 'overdue' ? <AlertCircle className="w-4 h-4" /> : <Clock className="w-4 h-4" />}
                </div>
                  ...
                  <div className="flex items-center justify-between gap-2">
                    ...
                    <Badge 
                      ...
                      className={cn(
                        ...
                        schedule.status === 'overdue' ? "text-destructive border-destructive/30" : "text-primary border-primary/30"
                      )}
                    >
                      {schedule.status === 'overdue' ? 'Atrasada' : 'Vencendo'}
                    </Badge>
                  </div>
                  <p className="text-[10px] text-muted-foreground truncate">{schedule.name}</p>
                  <p className="text-[9px] text-muted-foreground mt-0.5">
                    Vencimento: {formatDistanceToNow(new Date(schedule.next_maintenance), { addSuffix: true, locale: ptBR })}
                  </p>
                </div>
              </div>
            ))}
