...
function OccupancyChartComponent() {
  ...
  const occupancyData = useMemo(() => {
    ...
    return machines.map((machine: any) => {
      const machineJobs = jobs.filter((j: any) => j.machine_id === machine.id && activeStatuses.includes(j.status));
      const totalCapacityHours = 8;
      const usedHours = machineJobs.reduce((sum: number, j: any) => sum + (Number(j.estimated_duration) || 0) / 60, 0);
  ...
  const avgOccupancy = occupancyData.length > 0
    ? Math.round(occupancyData.reduce((sum: number, m: any) => sum + m.occupancy, 0) / occupancyData.length)
    : 0;
    ...
    <Card className="glass-card">
      ...
      <CardContent>
        <div className="space-y-3">
          {occupancyData.slice(0, 10).map((machine: any) => (
