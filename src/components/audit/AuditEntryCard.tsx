      ...
      <div className="flex items-start justify-between gap-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            {entry.action === 'status_change' && entry.new_values && typeof entry.new_values === 'object' && (
              <div className="flex items-center gap-2 mt-2">
                <span className="text-xs text-muted-foreground italic">Alterado para:</span>
                <Badge variant="outline" className={cn("text-[10px] uppercase font-bold", getStatusColor(String((entry.new_values as Record<string, any>).status || '')))}>
                  {String((entry.new_values as Record<string, any>).status || 'status')}
                </Badge>
              </div>
            )}
