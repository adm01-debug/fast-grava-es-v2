...
Deno.test("webhook-handler: should accept valid payload", async () => {
  const payload = {
    source: "bitrix24",
    event: "ONCRMDEALADD",
    data: { id: 123 }
...
Deno.test("webhook-handler: should reject missing source", async () => {
  const payload = {
    event: "ONCRMDEALADD",
    data: { id: 123 }
  };
...
Deno.test("webhook-handler: should reject invalid JSON", async () => {
  const req = new Request(mockUrl, {
    method: "POST",
    body: "not-json",
    headers: { "Content-Type": "application/json" }
